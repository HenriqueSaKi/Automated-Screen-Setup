...
class Telas_Monitoracao():
    ...
    def __init__(self):
        ...
        self.datas = [
            {"instituicao": "Hospital São Luiz São Caetano do Sul", "modelo": "EN - Consumo Energia Elétrica EE CAG"},
            {"instituicao": "Hospital São Luiz Itaim", "modelo": "EN - Efiiciencia Energética - Medidor CAG"},
            {"instituicao": "Hospital Unimed", "modelo": "EN - Consumo Energético - CAG"},
            {"instituicao": "Honda", "modelo": "EN - Eficiencia Energética CAG"}]
    ...
    def contratoManutencao_Tela1(self):
        ...
        resumo = WebDriverWait(self.driver, 20).until(
            EC.presence_of_element_located((By.XPATH, "//div[@id='nav']//li[2]")))
        resumo.click()
        time.sleep(3)
    ...
    def apply(self):
        applyButton = self.driver.find_element_by_xpath(
            "//div[@class='ui-dialog-buttonset']//button[contains(text(),'Aplicar')]")
        applyButton.click()
        time.sleep(3)

    def generatorButton(self):
        generator = self.driver.find_element_by_xpath(
            "//div[@class='ui-dialog-buttonset']//button[contains(text(),'Gerar')]")
        generator.click()
        time.sleep(2)
    ...
    def openReport(self):
        ...
        for i in range(4):
            ...
            reportModel.click()
            findModel = WebDriverWait(self.driver, 120).until(
    ...
    def GoogleAccess(self):
        self.driver.get(
            'https://chrome.google.com/webstore/detail/revolver-tabs/dlknooajieciikpedpldejhhijacnbda?hl=pt-BR')
    ...
    def AddExtension(self):
        ...
        addButton = WebDriverWait(self.driver, 20).until(
            EC.presence_of_element_located((By.XPATH, "//div[@class='g-c-x']//div[contains(text(),'Usar no Chrome')]")))
        addButton.click()
